...
def convert_time(seconds: int | float, decimal=15) -> str:
    ...
    if decimal == 0:
        ...
        seconds = round(seconds, decimal)

    if days:
        res.append(f'{days} day{"s"*(days != 1)}')

    if hours:
        res.append(f'{hours} hour{"s"*(hours != 1)}')

    if minutes:
        res.append(f'{minutes} minute{"s"*(minutes != 1)}')

    if seconds:
        res.append(f'{seconds} second{"s"*(seconds != 1)}')
