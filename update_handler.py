...
class UpdateHandler:
    ...
    async def initialize(self) -> None:
        ...
        site = TCPSite(web_app, '0.0.0.0', 7364)
        await site.start()
    ...
    async def handle_post(self, request: Request) -> Response:
        ...
        if (signature := request.headers.get("X-Hub-Signature-256", None)) is None:
            self.log.info(
                f'received post request from {request.remote} without x-hub-signature-256 header'
            )
            raise HTTPForbidden(
                reason='x-hub-signature-256 header is missing!'
            )
        # verify signature
        expected = new(self.secret.encode("utf-8"), msg=await request.read(), digestmod=sha256).hexdigest()

        if not compare_digest(f'sha256={expected}', signature):
            self.log.info(
                f'received post request from {request.remote} with invalid signature'
            )
            raise HTTPForbidden(reason='signature doesn\'t match!')

        self.log.info(
            f'received post request from {request.remote} with valid signature'
        )
        self._updating = True
    ...
    async def bot_start(self, bot_name: str) -> None:
        ...
        if not bot_data.enabled:
            self.log.info(
                f'skipping changes made to {bot_name} because it is disabled'
            )
            return
        ...
        match bot_data.type:
            case BotType.LARGE:
                self.bots.update(
                    {bot_name: ClientLarge(Project.model_validate(proj))})
            case BotType.SMALL:
                self.bots.update(
                    {bot_name: ClientSmall(Project.model_validate(proj))})
            case _:
                raise ValueError(f'invalid bot type {bot_data.type}')
    ...
    async def reload_extension(self, extension: str) -> None:
        for bot in self.bots.values():
            if extension in bot.project.bot.disabled_extensions:
                continue
            ...
            if extension.split('.') == 'bots':
                continue  # ? if it was a custom extension, it must either be disabled or was already loaded

            bot.load_extension(extension)

    async def change_monitor(self) -> None:
        if not self.base_project['config']['live_reload']:
            return
            ...
            for change_type, file_path in list(changes):
                ...
                match file_path.split('/'):
                    ...
                    case ['bots', bot_name, *additional]:
                        actions.add(
                            (self.reload_extension, f'bots.{bot_name}')
                        )
