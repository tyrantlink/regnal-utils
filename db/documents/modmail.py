...
class ModMail(Document):
    ...
    class ModMailMessage(BaseModel):
        author: Optional[int] = Field(
            default=None,
            description='author id, none for anonymous'
        )
        content: str = Field(
            max_length=4096,
        ...
        attachments: list[str] = Field(
            default=[],
            description='message attachment urls'
        )
        timestamp: int = Field(
    ...
    thread: Optional[int] = Field(default=None, description='thread id')
    send_all: bool = Field(
        default=False,
        description='whether every message is sent to the user, or just pings'
    )
    closed: bool = Field(
        default=False,
        description='whether the modmail is closed'
    )
    messages: list[ModMailMessage] = Field(
        default=[],
        description='modmail messages'
    )
